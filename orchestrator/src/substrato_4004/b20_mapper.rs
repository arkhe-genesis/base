...
fn extract_address(action: &Action, field: &str) -> Result<Address, MapperError> {
    if let Some(s) = action.payload.get(field).and_then(|v| v.as_str()) {
        Address::from_str(s)
            .map_err(|_| MapperError::ExtractionError("Invalid address".to_string()))
    } else {
        Ok(Address::default())
    }
}
fn extract_u256(_action: &Action, _field: &str) -> Result<U256, MapperError> {
    Ok(U256::default())
}
fn extract_optional_memo(_action: &Action) -> Result<Option<[u8; 32]>, MapperError> {
    Ok(None)
}
fn hash_memo(_prefix: &str, _action: &Action) -> [u8; 32] {
    [0; 32]
}
fn extract_policy_scope(_action: &Action) -> Result<PolicyScope, MapperError> {
    Ok(PolicyScope::TransferSender)
}
fn extract_u64(_action: &Action, _field: &str) -> Result<u64, MapperError> {
    Ok(0)
}
fn extract_pausable_features(_action: &Action) -> Result<Vec<PausableFeature>, MapperError> {
    Ok(vec![])
}
        ...
        match action.action_type.as_str() {
            "payment_b20" => {
                ...
                let sender_policy = self
                    ...
                    .await
                    .map_err(|e| MapperError::ExtractionError(format!("{:?}", e)))?;
