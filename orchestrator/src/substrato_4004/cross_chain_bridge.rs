...
use crate::substrato_4004::settlement_engine::{B20Payment, B20SettlementEngine, CrossChainEmitterV2};
...
impl EscrowManager {
    pub async fn get_state(&self, _id: &str) -> Result<EscrowState, BridgeError> {
        Ok(EscrowState { released: true, token: Address::default(), amount: ethers::types::U256::from(100) })
    }
}
...
impl X402XrplBridge {
    pub async fn create_settlement_escrow(&self, _payment: &B20Payment) -> Result<String, BridgeError> { Ok("escrow_id".to_string()) }
}
...
impl B20XrplBridge {
    ...
    pub async fn b20_to_xrpl_escrow(
        ...
        let compliance = self.b20_settlement.compliance_engine.evaluate_compliance(&action).await.map_err(|e| BridgeError::SettlementError(format!("{:?}", e)))?;
        ...
        }).await.map_err(|e| BridgeError::SettlementError(format!("{:?}", e)))?;
