...
def test_double_spend_detection(engine):
    ...
    # In reality, Alice would be attempting to spend using the same secret nullifier.
    # We mock the tx_id temporarily to generate a conflicting nullifier.
    import secrets
    original_tx_id = tx1.tx_id
    # Temporarily monkeypatch secrets.token_hex so we generate the exact same tx_id
    import unittest.mock
    with unittest.mock.patch('secrets.token_hex', return_value='00'*16):
        tx2_1 = engine.create_transaction("alice", "bob", 500)
