...
@dataclass
class AccountState:
    """Account state in the confidential ledger."""
    account_id: str
...
class ZKCBCC:
    ...
    def create_transaction(self, sender_priv: str, receiver_pub: str, amount: int) -> ConfidentialTransaction:
        ...
        # In ZK, the nullifier is derived from the secret (key) and the transaction id to be deterministic
        ...
        # Money supply preservation proof
    ...
    def audit_supply(self) -> Dict:
        """Audits the money supply without revealing individual transactions."""
        return {
...
╠══════════════════════════════════════════════════════════════════╣
  ...
  P1 - Diagnostics: Verifiable money supply without exposure
...
if __name__ == "__main__":
    ...
    try:
        # Simulate double spend using the same tx_id logic by forcing identical inputs.
        import unittest.mock
        with unittest.mock.patch('secrets.token_hex', return_value='00'*16):
            zk.create_transaction("alice", "carol", 500)
            zk.create_transaction("alice", "carol", 500)
        print("WARNING: Double spend not detected!")
    ...
    print(f"Tx3: {tx3.tx_id} | Status: {tx3.status.value} (rejected due to sanctions)")
