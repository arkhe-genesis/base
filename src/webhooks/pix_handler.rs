// src/webhooks/pix_handler.rs
//! Servidor Axum para receber webhooks de confirmação de pagamento Pix.
//! Processa notificações do gateway Pix e dispara a distribuição de royalties.
...
use crate::integrations::pix::{PixWebhookPayload, PixStatus, PixGateway};
...
impl PixWebhookHandler {
    ...
    pub async fn handle_webhook(
        ...
        // Verifica assinatura/autenticação (em produção: validar header)
        ...
        // Dispara a distribuição de royalties
    ...
    /// Verifica a assinatura do webhook (exemplo simplificado)
    async fn verify_signature(&self, _payload: &PixWebhookRequest) -> bool {
        // Em produção: validar com chave pública do gateway Pix
        // ou verificar token no header Authorization
        true // Placeholder
    }

    /// Processa a distribuição de royalties após confirmação do Pix
    async fn process_royalty_distribution(
        &self,
        dpid: &str,
        amount_brl: f64,
        payload: &PixWebhookRequest,
        ...
        // 2. Converte BRL → USDC (se necessário)
        let usdc_amount = if config.currency == "BRL" {
            // Usa taxa de câmbio atual (ex: via API)
            let rate = orchestrator.get_brl_usdc_rate().await?;
            (amount_brl / rate * 1_000_000.0) as u64 // USDC com 6 decimais
        } else {
            amount_brl as u64 // Já em USDC (assumindo)
        };

        // 3. Distribui via Picnic (se configurado)
        if let Some(basket) = &config.picnic_basket {
            ...
            picnic_manager.deposit_and_distribute(usdc_amount, &config.royalty_split).await?;
            return Ok("Distribuição via Picnic realizada".to_string());
        }

        // 4. Distribuição via Pix (Open Finance) — fallback
        ...
        if !pix_splits.is_empty() {
            ...
            for (pix_key, share) in pix_splits {
                let value = amount_brl * share as f64;
                recipients.push((pix_key, value));
            }
