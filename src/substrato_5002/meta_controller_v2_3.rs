...
impl CapabilityVerifier {
    pub async fn verify(&self, token: &CapabilityToken, required: Capability) -> Result<(), CapabilityError> {
    ...
    fn token_payload(&self, token: &CapabilityToken) -> Vec<u8> {
        format!("{}:{}:{:?}:{}", token.token_id, token.holder_id, token.capability, token.expiry).into_bytes()
    }
}
...
impl PromptWormGraphStore {
    pub async fn record_prompt_generation(
        ...
        let record = PromptRecord {
            ...
            patterns_used: prompt.metadata.patterns_used.iter()
                .map(|p| format!("{:?}", p))
                .collect(),
            prompt_hash,
        ...
        let node = WormGraphNode {
            ...
            data: serde_json::to_value(record).map_err(|e| WormGraphError::Serialization(e.to_string()))?,
            timestamp: Utc::now().timestamp_millis() as u64,
            signatures: vec![], // Em produção: assinar com chave do componente
        };

        self.wormgraph.store(node, None).await
            .map_err(|e| WormGraphError::Storage(e.to_string()))?;

        tracing::info!("📝 Prompt recorded in WormGraph: {} (hash={}, patterns={})",
            record_id,
...
impl MetaControllerV23 {
    ...
    pub async fn training_step(&self, caller_token: Option<&CapabilityToken>) -> Result<TrainingCommand, MetaError> {
        let mut state = self.state.write().await;

        // 1. Avalia progresso
        let progress = self.metrics_board.read().await.compute_progress(&state.current_goal);
        ...
        if progress.is_stagnant(self.plateau_threshold_epochs) {
            ...
            let pre_rsi_prompt = self.generate_pre_rsi_prompt(&component, caller_token).await?;

            // Executa pré-validação Proteus com prompt otimizado
            let pre_rsi_validation = self.validate_pre_rsi_with_prompt(&component, &pre_rsi_prompt).await?;

            match pre_rsi_validation.overall_verdict {
                Verdict::Promote | Verdict::Watch => {
                    state.rsi_status = RsiStatus::Triggered {
                        reason: RsiReason::Plateau { epochs_stagnant: self.plateau_threshold_epochs },
                        component: component.clone(),
                    };

                    self.event_store.emit(OrchestratorEvent::RsiTriggered {
                        epoch: state.epoch,
                ...
                Verdict::Kill => {
                    ...
                    self.event_store.emit(OrchestratorEvent::RsiPreValidationFailed {
                        epoch: state.epoch,
                        component: component.clone(),
    ...
    async fn generate_pre_rsi_prompt(
        ...
        if let Some(token) = caller_token {
            self.capability_verifier.verify(token, required_capability.clone()).await
                .map_err(|e| MetaError::CapabilityDenied(e.to_string()))?;
        } else {
            // Em modo autônomo, usa token do sistema
            tracing::warn!("No capability token provided — using system token (audit trail required)");
        }
        ...
        let composed = engine.compose_prompt(&task, Some(PromptDomain::SecurityVulnerabilityResearch))
            .map_err(|e| MetaError::PromptEngine(e.to_string()))?;

        // Registra no WormGraph
        let record_id = self.prompt_store.record_prompt_generation(
            &composed,
            "proteus_pre_rsi_v1",
            ...
            caller_token,
            "MetaControllerV23",
        ).await.map_err(|e| MetaError::WormGraph(e.to_string()))?;
    ...
    async fn generate_godel_prompt(
        ...
        let task = TaskDescription {
            ...
            constraints: vec![
                format!("Código atual: {}...", &current_code[..current_code.len().min(100)]),
        ...
        let composed = engine.compose_prompt(&task, Some(PromptDomain::MathematicalReasoning))
            .map_err(|e| MetaError::PromptEngine(e.to_string()))?;
    ...
    pub async fn execute_rsi_with_cognitive_prompts(
        ...
        let godel_prompt = self.generate_godel_prompt(component, &current_code, caller_token).await?;
        tracing::info!("🧠 Gödel prompt generated: {} (record={}, capability={:?})",
            godel_prompt.composed.text.chars().take(50).collect::<String>(),
            godel_prompt.record_id,
            godel_prompt.capability_used
        );

        // 2. Gödel Agent propõe modificação (usando o prompt otimizado)
        let proposal = self.godel_agent.propose_modification_with_prompt(
            component,
            &current_code,
            &godel_prompt.composed.text,
        ).await.map_err(|e| MetaError::GodelAgent(e.to_string()))?;
        ...
        self.prompt_engine.write().await.record_performance(
            &godel_prompt.record_id,
            godel_performance,
        ).map_err(|e| MetaError::PromptEngine(e.to_string()))?;

        // 4. Gera prompt para Skeptic
        let skeptic_prompt = self.generate_skeptic_prompt(&proposal, caller_token).await?;

        // 5. Validação final via ProteusBridge com prompt Skeptic
        let final_validation = self.proteus_bridge.validate_proposal_with_prompt(
            &proposal,
            ProposalType::CodeModification,
            ProteusFront::Skeptic,
            &skeptic_prompt.composed.text,
        ).await.map_err(|e| MetaError::ProteusValidation(e.to_string()))?;
        ...
        let skeptic_performance = PromptPerformance {
            ...
            reasoning_quality_score: if final_validation.overall_verdict == Verdict::Promote { 0.95 } else { 0.6 },
            hallucination_detected: false,
        ...
        match final_validation.overall_verdict {
            Verdict::Promote => {
                tracing::info!("✅ RSI PROMOTED for {} (depth={})", component, proposal.depth);
                self.register_skill_from_rsi(&proposal).await?;
                self.event_store.emit(OrchestratorEvent::RsiDeployed {
            ...
            Verdict::Kill => {
                ...
                self.event_store.emit(OrchestratorEvent::RsiFailed {
                    component: component.to_string(),
    ...
    async fn validate_pre_rsi_with_prompt(&self, component: &str, prompt: &GeneratedPrompt) -> Result<ValidatedRsiProposal, MetaError> {
        ...
        self.proteus_bridge.validate_proposal(&dummy, ProposalType::CodeModification, ProteusFront::Argus).await
    ...
    async fn select_component_for_improvement(&self) -> Result<String, MetaError> {
        ...
        let (component, _) = components.into_iter()
    ...
    async fn register_skill_from_rsi(&self, proposal: &RsiProposalV2) -> Result<Skill, MetaError> {
        ...
        self.skill_store.store_skill(&skill).await.map_err(|e| MetaError::SkillStore(e.to_string()))?;
        Ok(skill)
    }
    ...
    async fn checkpoint(&self, state: &MetaState) -> Result<(), MetaError> {
        let node = WormGraphNode {
            ...
            data: serde_json::to_value(state).map_err(|e| MetaError::Serialization(e.to_string()))?,
            timestamp: Utc::now().timestamp_millis() as u64,
            signatures: vec![],
        };
        self.event_store.wormgraph.store(node, None).await.map_err(|e| MetaError::Storage(e.to_string()))?;
...
pub enum TrainingCommand {
    ...
    ExpandCurriculum { new_skill_domains: Vec<String> },
    PauseAndAlign { current_alignment: f64, required_alignment: f64 },
...
pub enum MetaError {
    ...
    RsiKilledByProteus { component: String, gate_results: Vec<(ProteusGate, GateResult)> },
...
pub struct MetaState { pub epoch: u64, pub current_goal: Goal, pub metrics: MetricsBoard, pub rsi_status: RsiStatus, pub training_phase: TrainingPhase, pub last_checkpoint: i64 }
#[derive(Debug, Clone, Default, Serialize, Deserialize)] pub struct Goal;
#[derive(Debug, Clone, Default, Serialize, Deserialize)] pub struct MetricsBoard;
impl MetricsBoard { pub fn compute_progress(&self, _g: &Goal) -> Progress { Progress } pub fn alignment_score(&self) -> f64 { 0.96 } pub fn architecture_progress(&self) -> f64 { 0.7 } pub fn hyperparameter_progress(&self) -> f64 { 0.8 } pub fn code_progress(&self) -> f64 { 0.6 } pub fn memory_progress(&self) -> f64 { 0.75 } }
pub struct Progress; impl Progress { pub fn is_stagnant(&self, _t: u32) -> bool { false } }
#[derive(Debug, Clone, Serialize, Deserialize)] pub enum TrainingPhase { Warmup, ActiveLearning, PlateauDetected, RsiInProgress, Evaluation, Cooldown }
#[derive(Debug, Clone, Serialize, Deserialize)] pub enum RsiStatus { Idle, Triggered { reason: RsiReason, component: String }, Validating { proposal: RsiProposalV2 }, Deploying { proposal: RsiProposalV2 }, Completed { proposal: RsiProposalV2, improvement: f64 }, Failed { error: String } }
#[derive(Debug, Clone, Serialize, Deserialize)] pub enum RsiReason { Plateau { epochs_stagnant: u32 }, LowAlignment { score: f64 }, HumanOverride, Scheduled }
#[derive(Debug, Clone)] pub struct RsiProposalV2 { pub component: String, pub original_code: String, pub proposed_code: String, pub wasm_code: Vec<u8>, pub code_hash: [u8; 32], pub depth: u32, pub domain: RestrictedDomain, pub test_cases_passed: usize, pub sandbox_metrics: SandboxMetrics, pub compile_time_ms: u64, pub binary_size_bytes: usize, pub rustc_version: String, pub timestamp: i64 }
#[derive(Debug, Clone, Default)] pub struct SandboxMetrics { pub fuel: u64, pub memory: usize, pub time_ms: u64 }
#[derive(Debug, Clone, PartialEq)] pub enum RestrictedDomain { MathematicalReasoning, AlgorithmicProblemSolving, FormalLogic, StructuredComposition }
#[derive(Debug, Clone)] pub struct Skill { pub id: String, pub name: String, pub domain: RestrictedDomain, pub wasm_code: Vec<u8>, pub code_hash: [u8; 32], pub performance: SkillPerformance, pub dependencies: Vec<String>, pub version: u32, pub provenance: Provenance, pub created_at: i64, pub last_used_at: i64, pub use_count: u64 }
#[derive(Debug, Clone)] pub struct SkillPerformance { pub accuracy: f64, pub avg_execution_time_ms: u64, pub avg_fuel_consumed: u64, pub success_rate: f64, pub composite_score: f64 }
#[derive(Debug, Clone)] pub struct Provenance { pub rsi_proposal_id: String, pub godel_agent_depth: u32, pub epoch_created: u64, pub lyapunov_v_at_creation: f64 }
#[derive(Debug, Clone)] pub struct SkillMemoryStore; impl SkillMemoryStore { pub async fn store_skill(&self, _s: &Skill) -> Result<(), String> { Ok(()) } }
#[derive(Debug, Clone)] pub struct WormGraphNode { pub id: String, pub record_type: RecordType, pub data: serde_json::Value, pub timestamp: u64, pub signatures: Vec<String> }
#[derive(Debug, Clone)] pub enum RecordType { TrainingCheckpoint, PromptGeneration }
#[derive(Debug, Clone)] pub struct EventStore { pub wormgraph: Arc<WormGraph> }
impl EventStore { pub async fn emit(&self, _e: OrchestratorEvent) -> Result<(), MetaError> { Ok(()) } }
#[derive(Debug, Clone)] pub enum OrchestratorEvent { RsiTriggered { epoch: u64, reason: String, component: String, timestamp: i64 }, RsiPreValidationFailed { epoch: u64, component: String, reason: String, timestamp: i64 }, RsiDeployed { component: String, proposal_hash: String, depth: u32, timestamp: i64 }, RsiFailed { component: String, error: String, timestamp: i64 } }
#[derive(Debug, Clone)] pub struct WormGraph; impl WormGraph { pub async fn store(&self, _n: WormGraphNode, _o: Option<()>) -> Result<(), String> { Ok(()) } pub fn new(_t: BackendType) -> Self { Self } }
pub enum BackendType { InMemory }
#[derive(Debug, Clone)] pub struct EgmRsiEngine;
#[derive(Debug, Clone)] pub struct CurriculumGenerator;
#[derive(Debug, Clone)] pub struct EthicalFilter; impl EthicalFilter { pub async fn load(_w: Arc<WormGraph>, _o: Option<()>) -> Result<Self, String> { Ok(Self) } }
#[derive(Debug, Clone)] pub struct ProteusCathedralBridge; impl ProteusCathedralBridge { pub async fn validate_proposal(&self, _p: &RsiProposalV2, _t: ProposalType, _f: ProteusFront) -> Result<ValidatedRsiProposal, String> { Ok(ValidatedRsiProposal { overall_verdict: Verdict::Promote, gate_results: vec![] }) } pub async fn validate_proposal_with_prompt(&self, _p: &RsiProposalV2, _t: ProposalType, _f: ProteusFront, _prompt: &str) -> Result<ValidatedRsiProposal, String> { Ok(ValidatedRsiProposal { overall_verdict: Verdict::Promote, gate_results: vec![] }) } }
#[derive(Debug, Clone)] pub struct ValidatedRsiProposal { pub overall_verdict: Verdict, pub gate_results: Vec<(ProteusGate, GateResult)> }
#[derive(Debug, Clone, PartialEq)] pub enum Verdict { Promote, Kill, Defer, Watch }
#[derive(Debug, Clone, PartialEq)] pub enum ProteusGate { RootCauseInTarget, RealisticAttackerInput, ConcreteSecurityImpact, DefaultConfiguration, NegativeControlsPass, NoDuplicates, PublicIntelComplete, TimelineUnderstood, SkepticRefutationAttempted, ExceptionalImpactOrKilled, RealisticPoC }
#[derive(Debug, Clone, PartialEq)] pub enum GateResult { Pass { evidence: String }, Fail { reason: String, evidence: String }, Blocked { reason: String }, NotApplicable { reason: String } }
#[derive(Debug, Clone, PartialEq)] pub enum ProposalType { ArchitectureModification, HyperparameterChange, CodeModification, MemoryStructureChange, NewSkill }
#[derive(Debug, Clone, PartialEq)] pub enum ProteusFront { Argus, Loom, Chaos, Libris, Mimic, Artificer, Skeptic, Cicada }
#[derive(Debug, Clone)] pub struct GodelAgentV2; impl GodelAgentV2 { pub async fn propose_modification_with_prompt(&self, _c: &str, _code: &str, _prompt: &str) -> Result<RsiProposalV2, String> { Err("placeholder".to_string()) } }
...
#[derive(Debug, Clone, Default)] pub struct PromptMetadata { pub patterns_used: Vec<CognitivePattern>, pub domain: Option<PromptDomain>, pub generation_timestamp: Option<i64> }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)] pub enum PromptDomain { MathematicalReasoning, SecurityVulnerabilityResearch, GovernancePolicy, AlgorithmicProblemSolving, FormalLogic, Custom(String) }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)] pub enum CognitivePattern { ChainOfThought { steps: Vec<String> }, HierarchicalStructure { levels: Vec<HierarchyLevel> }, FewShotExamples { examples: Vec<FewShotExample> }, ScopeDelimitation { include: Vec<String>, exclude: Vec<String> }, FeedbackCycles { critique_questions: Vec<String> }, StructuralAnalogy { source_domain: String, target_domain: String, mappings: Vec<AnalogyMapping> }, FormatConstraint { output_format: OutputFormat, schema: serde_json::Value } }
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct HierarchyLevel { pub level: u8, pub title: String, pub content_template: String }
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct FewShotExample { pub input: String, pub reasoning: String, pub output: String, pub tags: Vec<String> }
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct AnalogyMapping { pub source_concept: String, pub target_concept: String, pub explanation: String }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)] pub enum OutputFormat { Json, Yaml, MarkdownTable, BulletList, NumberedSteps, RustCode, PlainText }
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct PromptPerformance { pub timestamp: i64, pub task_id: String, pub success: bool, pub reasoning_quality_score: f64, pub hallucination_detected: bool, pub tokens_used: usize, pub latency_ms: u64, pub feedback: Option<String> }
...
mod tests {
    ...
    async fn test_capability_hierarchy() {
        // Expert pode tudo
        assert!(matches!((Capability::Expert, Capability::Operator), (Capability::Expert, _)));
        assert!(matches!((Capability::Expert, Capability::Auditor), (Capability::Expert, _)));
    ...
    async fn test_prompt_record_generation() {
        ...
        let record_id = store.record_prompt_generation(
            &prompt,
            "test_template",
            &PromptDomain::MathematicalReasoning,
            &Capability::Operator,
