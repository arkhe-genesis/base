...
impl CompensationPromptIntegrator {
    ...
    pub async fn record_compensation_failure(
        ...
        let failure_id = format!("comp_fail_{}_{}", compensation_tx_id, Utc::now().timestamp_millis());

        // 1. Analisa falha e propõe recovery
        let recovery = self.analyze_failure_and_propose_recovery(
            &failure_type,
            &error_details,
            &context,
        ).await?;
        ...
        tracing::info!(
            ...
            failure_id, failure_type, recovery.recovery_type
        );
    ...
    async fn analyze_failure_and_propose_recovery(
        ...
        match failure_type {
            CompensationFailureType::PromptHallucination => {
                Ok(RecoveryProposal {
                    recovery_type: RecoveryType::PromptRegeneration,
                    description: "Regenerar prompt com mais few-shot examples e CoT explícito".to_string(),
                    requires_rsi: false,
                    rsi_target: None,
                    estimated_risk: RiskLevel::Low,
                })
            }
            CompensationFailureType::PromptScopeViolation => {
                Ok(RecoveryProposal {
            ...
            CompensationFailureType::PromptFormatError => {
                Ok(RecoveryProposal {
                    recovery_type: RecoveryType::PromptRefinement,
            ...
            CompensationFailureType::CompensationExecutionFail => {
                ...
                Ok(RecoveryProposal {
                    ...
                    description: "Acionar RSI para melhorar gerador de código de compensação".to_string(),
                    requires_rsi: true,
                    rsi_target: Some("compensation_code_generator".to_string()),
                    estimated_risk: RiskLevel::Medium,
                })
            }
            CompensationFailureType::Timeout => {
                Ok(RecoveryProposal {
            ...
            CompensationFailureType::SafetyViolation => {
                ...
                Ok(RecoveryProposal {
                    recovery_type: RecoveryType::RsiCodeImprovement,
                    description: "Acionar RSI com validação Skeptic reforçada + auditoria de segurança".to_string(),
                    requires_rsi: true,
    ...
    async fn penalize_prompt_patterns(
        ...
        tracing::warn!(
            ...
            prompt_record_id, penalty, failure_type
        );
    ...
    pub async fn generate_compensation_prompt(
        ...
        // Verifica capacidade
        if caller_token.capability != Capability::Expert && caller_token.capability != Capability::Operator {
            return Err(CompensationError::CapabilityDenied(
                format!("Compensation generation requires Operator+, got {:?}", caller_token.capability)
            ));
        }
        ...
        let task = TaskDescription {
            ...
            constraints: vec![
                format!("Montante: {} {}", compensation_request.amount, compensation_request.asset),
        ...
        let composed = engine.compose_prompt(&task, Some(PromptDomain::Custom("compensation".to_string())))
            .map_err(|e| CompensationError::PromptEngine(e.to_string()))?;

        let record_id = format!("comp_prompt_{}_{}", compensation_request.tx_id, Utc::now().timestamp_millis());
...
// Placeholder types
#[derive(Debug, Clone)] pub struct CognitivePromptEngine; impl CognitivePromptEngine { pub fn compose_prompt(&self, _task: &TaskDescription, _domain: Option<PromptDomain>) -> Result<ComposedPrompt, String> { Ok(ComposedPrompt { text: "test".to_string(), metadata: PromptMetadata::default(), estimated_tokens: 100 }) } }
...
#[derive(Debug, Clone)] pub struct GeneratedPromptV3 { pub composed: ComposedPrompt, pub record_id: String, pub capability_used: Capability, pub task_type: CognitiveTaskType }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)] pub enum CognitiveTaskType { Planning, CodeGeneration, SecurityAudit, GovernanceReview, DataAnalysis, ToolExecution, Reflection, Custom(String) }
...
#[derive(Debug, Clone)] pub struct MetaControllerV23;
...
#[derive(Debug, Clone)] pub enum RecordType { CompensationFailure, CompensationPrompt }
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct CognitiveTask { pub id: String, pub task_type: CognitiveTaskType, pub objective: String, pub constraints: Vec<String>, pub priority: f64, pub deadline: Option<i64>, pub metadata: HashMap<String, String> }
...
mod tests {
    ...
    async fn test_failure_analysis() {
        ...
        let recovery = integrator.analyze_failure_and_propose_recovery(
            &CompensationFailureType::CompensationExecutionFail,
        ...
        assert_eq!(recovery.rsi_target, Some("compensation_code_generator".to_string()));
    ...
    async fn test_compensation_prompt_generation() {
        ...
        let prompt = integrator.generate_compensation_prompt(&request, &token).await.unwrap();
