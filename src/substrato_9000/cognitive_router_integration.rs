...
impl CognitiveRouterV3 {
    ...
    pub async fn execute_cognitive_task(
        ...
        tracing::info!("🧠 CognitiveTask started: {} (type={:?})", task.id, task.task_type);

        // 1. VERIFICA CAPACIDADE
        let required_capability = self.task_type_to_capability(&task.task_type);
        self.capability_verifier.verify(caller_token, required_capability).await
        ...
        let plan_prompt = self.generate_planning_prompt(&task, &memories, prompt_domain.clone(), caller_token).await?;

        // 5. EXECUTA PLANEJAMENTO ReAct COM PROMPT OTIMIZADO
        let plan = self.react_engine.plan(&plan_prompt.composed.text, &task).await
        ...
        let prompt_record_id = self.record_prompt_in_wormgraph(
            &plan_prompt,
            &task,
            "planning",
            caller_token,
        ...
        for (i, step) in plan.steps.iter().enumerate() {
            tracing::info!("▶️  Executing step {}/{}: {}", i + 1, plan.steps.len(), step.description);

            let step_prompt = self.generate_step_prompt(&task, step, &memories, prompt_domain.clone()).await?;
        ...
        let reflection_prompt = self.generate_reflection_prompt(&task, &plan, &step_results).await?;
        let reflection = self.react_engine.reflect(&reflection_prompt.composed.text).await
        ...
        self.update_prompt_engine_performance(&plan_prompt, &result).await?;

        tracing::info!("✅ CognitiveTask completed: {} (success={}, time={}ms)",
            task.id, result.success, result.execution_time_ms);
    ...
    async fn generate_planning_prompt(
        ...
        let task_desc = TaskDescription {
            ...
            constraints: vec![
                ...
                format!("Ferramentas disponíveis: {}", self.tool_registry.list_tools().join(", ")),
        ...
        let composed = engine.compose_prompt(&task_desc, Some(domain))
    ...
    async fn generate_step_prompt(
        ...
        let tool_info = self.tool_registry.get_tool(&step.tool_name)
    ...
    async fn generate_reflection_prompt(
        ...
        let failure_count = step_results.len() - success_count;

        let task_desc = TaskDescription {
            description: format!(
                "Reflexão pós-execução da tarefa: {}\nResultado: {}/{} passos bem-sucedidos",
                task.id, success_count, step_results.len()
            ),
            constraints: vec![
        ...
        let composed = engine.compose_prompt(&task_desc, Some(PromptDomain::GovernancePolicy))
            .map_err(|e| CognitiveRouterError::PromptEngine(e.to_string()))?;
    ...
    async fn execute_step(
        ...
        // Recupera ferramenta
        let tool = self.tool_registry.get_tool(&step.tool_name)
        ...
        if let Some(required) = &tool.required_capability {
            self.capability_verifier.verify(caller_token, required.clone()).await
                .map_err(|e| CognitiveRouterError::CapabilityDenied(format!(
                    "Tool '{}' requires {:?}: {}", step.tool_name, required, e
                )))?;
        }

        // Executa ferramenta com prompt otimizado
        let result = tool.execute(&step.parameters, &prompt.composed.text).await
    ...
    async fn store_task_result(
        ...
        let node = WormGraphNode {
            ...
            data: serde_json::to_value(result).map_err(|e| CognitiveRouterError::Serialization(e.to_string()))?,
            timestamp: Utc::now().timestamp_millis() as u64,
        ...
        self.episodic_memory.store(EpisodicMemory {
            task_id: result.task_id.clone(),
            success: result.success,
            plan_quality: result.reflection.plan_quality_score,
            timestamp: result.timestamp,
        }).await.map_err(|e| CognitiveRouterError::Memory(e.to_string()))?;
    ...
    async fn update_prompt_engine_performance(
        ...
        engine.record_performance(&prompt.record_id, performance)
            .map_err(|e| CognitiveRouterError::PromptEngine(e.to_string()))?;
    ...
    async fn retrieve_relevant_memories(&self, task: &CognitiveTask) -> Result<Vec<EpisodicMemory>, CognitiveRouterError> {
        // Busca memórias similares via zVEC
        let query = format!("{:?} {}", task.task_type, task.objective);
        let memories = self.semantic_memory.search(&query, 5).await
...
// Placeholder types
#[derive(Debug, Clone)] pub struct ReActEngine; impl ReActEngine { pub async fn plan(&self, _prompt: &str, _task: &CognitiveTask) -> Result<ExecutionPlan, String> { Ok(ExecutionPlan { steps: vec![], estimated_cost: 0.0, risk_level: RiskLevel::Low }) } pub async fn reflect(&self, _prompt: &str) -> Result<ReflectionResult, String> { Ok(ReflectionResult { summary: "ok".to_string(), plan_quality_score: 0.9, hallucination_detected: false, lessons_learned: vec![], suggested_improvements: vec![] }) } }
#[derive(Debug, Clone)] pub struct ToolRegistry; impl ToolRegistry { pub fn list_tools(&self) -> Vec<String> { vec!["search".to_string(), "calc".to_string()] } pub fn get_tool(&self, name: &str) -> Option<Tool> { Some(Tool { name: name.to_string(), description: "test".to_string(), required_capability: None }) } }
#[derive(Debug, Clone)] pub struct Tool { pub name: String, pub description: String, pub required_capability: Option<Capability> } impl Tool { pub async fn execute(&self, _params: &HashMap<String, String>, _prompt: &str) -> Result<ToolResult, String> { Ok(ToolResult { success: true, output: "ok".to_string(), execution_time_ms: 100 }) } }
#[derive(Debug, Clone)] pub struct ToolResult { pub success: bool, pub output: String, pub execution_time_ms: u64 }
#[derive(Debug, Clone)] pub struct EpisodicMemoryStore; impl EpisodicMemoryStore { pub async fn store(&self, _m: EpisodicMemory) -> Result<(), String> { Ok(()) } }
#[derive(Debug, Clone)] pub struct EpisodicMemory { pub task_id: String, pub success: bool, pub plan_quality: f64, pub timestamp: i64 }
#[derive(Debug, Clone)] pub struct SemanticMemoryStore; impl SemanticMemoryStore { pub async fn search(&self, _query: &str, _k: usize) -> Result<Vec<EpisodicMemory>, String> { Ok(vec![]) } }
#[derive(Debug, Clone)] pub struct CognitivePromptEngine; impl CognitivePromptEngine { pub fn compose_prompt(&self, _task: &TaskDescription, _domain: Option<PromptDomain>) -> Result<ComposedPrompt, String> { Ok(ComposedPrompt { text: "test".to_string(), metadata: PromptMetadata::default(), estimated_tokens: 100 }) } pub fn record_performance(&mut self, _id: &str, _perf: PromptPerformance) -> Result<(), String> { Ok(()) } }
...
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct TaskDescription { pub description: String, pub constraints: Vec<String>, pub priority: f64 }
...
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct CapabilityToken { pub token_id: String, pub holder_id: String, pub capability: Capability, pub expiry: i64, pub issued_by: String, pub signature: Vec<u8> }
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)] pub enum Capability { Expert, Operator, Auditor, Researcher, Guest }
#[derive(Debug, Clone)] pub struct CapabilityVerifier; impl CapabilityVerifier { pub async fn verify(&self, _token: &CapabilityToken, _required: Capability) -> Result<(), String> { Ok(()) } }
#[derive(Debug, Clone)] pub struct WormGraph; impl WormGraph { pub async fn store(&self, _n: WormGraphNode, _o: Option<()>) -> Result<(), String> { Ok(()) } }
...
#[derive(Debug, Clone)] pub enum RecordType { PromptGeneration, CognitiveTaskResult }
...
mod tests {
    ...
    async fn test_task_type_mapping() {
        ...
        assert_eq!(router.task_type_to_capability(&CognitiveTaskType::Planning), Capability::Expert);
        assert_eq!(router.task_type_to_capability(&CognitiveTaskType::SecurityAudit), Capability::Auditor);
        assert_eq!(router.task_type_to_capability(&CognitiveTaskType::ToolExecution), Capability::Operator);

        assert_eq!(router.task_type_to_prompt_domain(&CognitiveTaskType::CodeGeneration), PromptDomain::MathematicalReasoning);
        assert_eq!(router.task_type_to_prompt_domain(&CognitiveTaskType::SecurityAudit), PromptDomain::SecurityVulnerabilityResearch);
    }
