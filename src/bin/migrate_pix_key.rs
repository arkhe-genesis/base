...
async fn main() -> Result<(), String> {
    ...
    // 1. Carrega o HashTree
    let storage = HashTreeStorage::new(&args.hashtree_path)
        .map_err(|e| format!("Erro ao carregar HashTree: {}", e))?;

    // 2. Carrega o mapeamento npub -> pix_key (se fornecido)
    let mapping = if let Some(path) = &args.mapping_file {
        load_mapping(path).await?
    } else {
        // Se não fornecido, usa um mapeamento interativo ou padrão
        info!("ℹ️ Nenhum arquivo de mapeamento fornecido. Será usado um prompt interativo.");
        HashMap::new()
    };

    // 3. Lista todos os Nodes no HashTree
    ...
    for entry in entries {
        // Cada entry é um diretório de Node (ex: desci/nodes/desci:node:abc123/)
        let node_id = entry.trim_start_matches(nodes_path).trim_end_matches('/');
        info!("📂 Processando Node: {}", node_id);

        // Carrega a versão mais recente do Node
        ...
        // Obtém pix_key para cada contribuidor
        ...
        for contributor in &mut node.contributors {
            if let Some(pix_key) = mapping.get(&contributor.npub) {
                ...
                // Interativo: pergunta ao usuário
                if !args.dry_run {
...
/// Carrega mapeamento de um arquivo CSV (npub,pix_key)
