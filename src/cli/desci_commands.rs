...
impl DeSciCommand {
    pub fn parse(input: &str) -> Option<Self> {
        ...
        match parts[0] {
            "/desci-publish" | "desci-publish" => {
                ...
                while i < parts.len() {
                    match parts[i] {
                        "--title" => { if i + 1 < parts.len() { title = parts[i + 1].to_string(); i += 2; } else { i += 1; } }
                        "--abstract" => { if i + 1 < parts.len() { abstract_text = Some(parts[i + 1].to_string()); i += 2; } else { i += 1; } }
                        "--components" => { if i + 1 < parts.len() { components = parts[i + 1].split(',').map(|s| s.to_string()).collect(); i += 2; } else { i += 1; } }
                        "--authors" => { if i + 1 < parts.len() { authors = parts[i + 1].split(',').map(|s| s.to_string()).collect(); i += 2; } else { i += 1; } }
                        "--license" => { if i + 1 < parts.len() { license = Some(parts[i + 1].to_string()); i += 2; } else { i += 1; } }
    ...
    pub async fn execute(&self, orchestrator: &mut SecondSelfOrchestrator) -> Result<String, String> {
        match self {
            Self::Publish { title, spdx_license, copyright_holder, software_version, derived_from, ai_generated, training_data, publish, .. } => {
                ...
                node.ai_generated = *ai_generated;
                node.training_data_provenance = training_data.clone();

                let dpid = orchestrator.publish_desci_node(&mut node, *publish).await?;
                Ok(format!("Publicado: {}", dpid))
            }
