...
pub struct BittensorClient {
    pub config: BittensorConfig,
    pub client: Client,
    // Cache de axons por subnet
    pub axons_cache: Arc<Mutex<HashMap<u16, Vec<Axon>>>>,
}
...
impl BittensorClient {
    ...
    pub async fn query_subnet_with_fallback<T: Serialize + Clone + Send + Sync + 'static, R: for<'de> Deserialize<'de> + Send + 'static>(
        &self,
        subnet_id: u16,
        endpoint: &str,
        payload: &T,
