...
impl Check for SafetyCheck {
    ...
    async fn execute(&self, ctx: &FileContext) -> anyhow::Result<CheckResult> {
        ...
        let score = 1.0 - all_issues.iter()
            .map(|i| match i.severity { Severity::Error => 0.3, Severity::Warning => 0.1, _ => 0.05 })
            .sum::<f64>()
            .min(1.0);
