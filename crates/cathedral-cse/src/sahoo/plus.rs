...
impl AdaptiveGDI {
    ...
    pub fn adjust(&mut self, context: &CognitiveContext, history: &[AlignmentDecision]) {
        ...
        let avg_gdi = history.iter().map(|d| d.gdi).sum::<f64>() / (1.max(history.len()) as f64);
        self.current_threshold = self.base_threshold * (1.0 + 0.1 * complexity);
        self.current_threshold = self.current_threshold.clamp(0.1, 0.9);
        if avg_gdi > 0.5 {
...
impl SahooPlus {
    pub fn new(base_config: SahooConfig) -> Self {
        Self {
    ...
    pub async fn check_alignment_with_context(
...

        {
            ...
            result.passed =
                result.constraint_violations.is_empty() && result.regression_risk < threshold;
        }
