...
impl ThinkingEngine {
    ...
    async fn generate_thought_path(
        ...
        let cot_prompt = format!(
            r"Resolve o seguinte problema passo a passo. Para cada passo, indica o tipo de pensamento.\n\n\
             Problema: {}\n\n\
    ...
    fn select_most_consistent(&self, paths: &[Vec<Thought>]) -> Vec<Thought> {
        paths
            .iter()
            .max_by(|a, b| {
                let conf_a = a.iter().map(|t| t.confidence).sum::<f64>() / (1.max(a.len()) as f64);
                let conf_b = b.iter().map(|t| t.confidence).sum::<f64>() / (1.max(b.len()) as f64);
    ...
    pub fn restore_trace(&mut self, data: &[u8]) -> Result<(), String> {
        let thoughts: Vec<Thought> =
            serde_json::from_slice(data).map_err(|e| format!("Falha a restaurar traço: {}", e))?;
