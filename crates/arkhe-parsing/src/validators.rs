...
pub fn validate_did(did: &str) -> ParseResult<()> {
    ...
    if !re.is_match(did)? {
        return Err(ParseError::InvalidFormat(
            "DID must match did:arkhe:<43-44 base58 chars>".into(),
        ));
    }
    if has_invisible_unicode(did) {
        return Err(ParseError::SuspiciousCharacters(
            "DID contains invisible or control characters".into(),
        ));
    }
    if did.contains('\0') {
    ...
    if parts.len() != 3 || parts[0] != "did" || parts[1] != "arkhe" {
        return Err(ParseError::InvalidFormat(
            "DID must have exactly 3 parts: did:arkhe:<id>".into(),
        ));
    }
    Ok(())
}
...
pub fn validate_iban(iban: &str) -> ParseResult<()> {
    ...
    if !re.is_match(&cleaned)? {
        return Err(ParseError::InvalidFormat(
            "IBAN must match [A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}".into(),
        ));
    }
    if cleaned.len() < 5 {
...
pub fn validate_bic(bic: &str) -> ParseResult<()> {
    ...
    if !re.is_match(bic)? {
        return Err(ParseError::InvalidFormat(
            "BIC must be 8 or 11 characters: BBBBCCLL[BBB]".into(),
        ));
    }
