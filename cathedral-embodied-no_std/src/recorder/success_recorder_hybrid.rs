...
use crate::recorder::success_recorder_json::JsonRecorder; // JSON
...
impl SuccessRecorder {
    ...
    pub fn new() -> Self {
        ...
        if let Ok(db_path) = std::env::var("SUCCESS_RECORDER_DB") {
            if let Ok(sqlite) = SqliteRecorder::new(&db_path) {
                ...
                return Self { backend: RecorderBackend::Sqlite(sqlite) };
            } else {
                eprintln!(
                    "[SuccessRecorder] Falha ao abrir SQLite em {}. Usando JSON como fallback.",
                    db_path
                );
            }
        }
        ...
        Self { backend: RecorderBackend::Json(json_recorder) }
    }
    ...
    pub fn average_acceptance_rate(&self, last_n: Option<u32>) -> f32 {
        match &self.backend {
            ...
            RecorderBackend::Json(r) => r.average_acceptance_rate(last_n.map(|n| n as usize)),
        }
    }
